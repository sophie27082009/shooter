...
display.set_icon(image.load("shooter/asteroidd.png"))

background = transform.scale(image.load("shooter/galaxyy.jpg"), (WIDTH, HEIGHT))
clock = time.Clock()
...
class Player(GameSprite):
    def __init__(self, x, y):
        super().__init__("shooter/rocket.png", x, y, 100, 150, 10)
        self.kd = 0
        self.hp = 3
        self.clip = 10
        self.delay = 8
...
class Ufo(GameSprite):
    def __init__(self):
        ...
        super().__init__("shooter/ufoo.png", randint(0, WIDTH - 140), randint(-HEIGHT, - 70), 130, 90, 2)
...
class BossUfo(GameSprite):
    def __init__(self):
        ...
        super().__init__("shooter/ufoo.png", self.x, self.y, 200, 100, 1)
...
class Bullet(GameSprite):
    def __init__(self, x, y):
        super().__init__("shooter/bullett.png", x, y, 20, 40, 15)
...
class HealthBonus(Bonus):
    def __init__(self, x, y):
        super().__init__("shooter/health.png", x, y)
    ...
    def update(self):
        ...
        if self.rect.colliderect(player.rect):
            ...
            heart = GameSprite("shooter/health.png", heart_x, 20, 50, 50, 0)
            player.hp += 1
            health.append(heart)
            self.kill()
...
class Asteroid(GameSprite):
    def __init__(self):
        ...
        super().__init__("shooter/asteroidd.png", self.x, self.y, self.size, self.size, 3)
    ...
    def spawn(self):
        self.size = randint(30, 70)
        self.image = transform.scale(image.load("shooter/asteroidd.png"), (self.size, self.size))
        self.rect = self.image.get_rect()
        self.rect.x = randint(0, WIDTH - 140)
        self.rect.y = randint(-HEIGHT, - 70)
...
for i in range(player.clip):
    cartridge = GameSprite("shooter/bullett.png", cartridge_x, HEIGHT - 60, 20, 40, 0)
